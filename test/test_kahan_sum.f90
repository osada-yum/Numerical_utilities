program test_kahan_sum
  ...
  real(real64) :: summ, summ_kahan_sum
  ...
  write(error_unit, '(a)') "[test_kahan_sum]"
  write(error_unit, '(g0)') abs(summ - exact_sum)
  write(error_unit, '(g0)') abs(summ_kahan_sum - exact_sum)
  write(error_unit, '(g0)') abs(summ_k%val() - exact_sum)
end program test_kahan_sum
